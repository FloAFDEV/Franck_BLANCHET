...
// URL de l'image du canard fournie
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setView('PRACTITIONER_PROFILE')} className="flex items-center gap-3 p-1.5 pr-4 bg-slate-100 dark:bg-slate-800 rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all group">
              ...
              <div className={`w-9 h-9 rounded-xl border-2 border-white dark:border-slate-700 overflow-hidden shadow-sm ${practitioner?.photo === DUCK_AVATAR ? 'bg-primary p-0.5' : ''}`}>
                {practitioner?.photo ? (
                  <img src={practitioner.photo} alt="" className={`w-full h-full ${practitioner.photo === DUCK_AVATAR ? 'object-contain' : 'object-cover'}`} />
                ) : (
                  <UserCircle size={24} className="m-auto text-slate-300" />
                )}
              </div>
