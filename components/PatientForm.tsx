...
const InputField = ({ label, value, onChange, type = "text", placeholder = "", required = false, id, helperText = "" }: any) => (
  ...
  const capturePhoto = async () => {
    if (videoRef.current) {
      ...
      canvas.toBlob(async (blob) => {
        if (blob) { try { const id = await processAndStoreImage(blob); setPhotoId(id); } catch(e: any) { alert(e.message); } }
