...
import { User, Phone, Briefcase, Calendar, History, Plus, Edit3, Trash2, ChevronDown, Mail, Download, MapPin, Users, Activity, HeartPulse, Stethoscope, Eye, Ear, ClipboardList, BookOpen } from 'lucide-react';
...
const PatientDetail: React.FC<PatientDetailProps> = ({ patientId, onEdit, onDelete }) => {
  ...
  const SectionHeader = ({ icon: Icon, title }: any) => (
    <div className="flex items-center gap-2 mb-4 text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] px-1">
  ...
  const MedicalCard = ({ title, content, icon: Icon }: any) => (
    <div className="bg-white dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col gap-3">
      ...
      <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed italic">{content || "Néant"}</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-300">
      {/* Profil Header */}
      <div className="bg-white dark:bg-slate-900 p-6 sm:p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-lg flex flex-col md:flex-row gap-8 items-center md:items-start print:shadow-none print:border-none">
        <div className={`w-36 h-36 sm:w-44 sm:h-44 rounded-3xl border-4 ${patient.gender === 'F' ? 'border-pink-50' : 'border-blue-50'} dark:border-slate-800 bg-slate-50 dark:bg-slate-800 overflow-hidden shrink-0 shadow-md flex items-center justify-center text-slate-100`}>
        ...
        <div className="flex-1 w-full text-center md:text-left">
          <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-4 mb-6">
            <div>
              <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-100 leading-none">
                {patient.lastName} <span className="font-normal text-slate-500">{patient.firstName}</span>
              </h2>
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-3">
                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${patient.gender === 'F' ? 'bg-pink-100 text-pink-600' : 'bg-blue-100 text-blue-600'}`}>{patient.gender === 'M' ? 'Homme' : 'Femme'} • {age} ans</span>
                <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-full text-[10px] font-medium text-slate-500 uppercase tracking-wider flex items-center gap-1.5"><Calendar size={12} /> {new Date(patient.birthDate).toLocaleDateString()}</span>
                {patient.isSmoker && <span className="px-2.5 py-1 bg-amber-100 text-amber-600 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5"><Activity size={12} /> Fumeur</span>}
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={handleDelete} className="p-2.5 bg-rose-50 dark:bg-rose-900/20 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-sm"><Trash2 size={18} /></button>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <InfoTag label="Famille" value={`${patient.familyStatus} / ${patient.hasChildren}`} icon={Users} />
          ...
          <section>
            ...
            <div className="space-y-3">
              <div className="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Activité Sportive</p>
                <p className="text-sm text-slate-700 dark:text-slate-300 font-medium">{patient.physicalActivity || "Aucune"}</p>
              </div>
              <div className="p-5 bg-primary-soft border border-primary-border rounded-3xl">
                <div className="flex items-center gap-2 mb-2 text-[10px] font-bold text-primary uppercase tracking-widest"><Stethoscope size={14} /> Médecin Traitant</div>
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{patient.gpName || "Non déclaré"}</p>
                <p className="text-[10px] font-medium text-slate-500 mt-1 uppercase">{patient.gpCity}</p>
              </div>
              <div className="p-5 bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/50 rounded-3xl">
                <div className="flex items-center gap-2 mb-2 text-[10px] font-bold text-amber-600 uppercase tracking-widest"><Activity size={14} /> Traitement en cours</div>
                <p className="text-sm text-amber-900 dark:text-amber-200 leading-relaxed italic">{patient.currentTreatment || "Néant"}</p>
              </div>
            </div>
          </section>
        </div>
      ...
      <section className="pt-8 border-t border-slate-100 dark:border-slate-800 space-y-6">
        <div className="flex items-center justify-between px-1">
          ...
          <button onClick={() => setIsAddingSession(true)} className="flex items-center gap-2 bg-primary text-white px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-wider shadow-md hover:brightness-105 transition-all">
                ...
                <div onClick={() => toggleSession(s.id!)} className="px-6 py-4 flex items-center justify-between cursor-pointer group">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-slate-800 dark:text-slate-100">{new Date(s.date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' })}</span>
                      <span className="text-[10px] font-medium text-slate-400 uppercase tracking-tight truncate max-w-xs">{s.hdlm}</span>
                    </div>
                  </div>
                  <ChevronDown size={20} className={`text-slate-300 transition-transform ${open ? 'rotate-180 text-primary' : ''}`} />
                </div>
                {open && (
                  <div className="px-6 pb-8 pt-4 grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-50 dark:border-slate-800 animate-in fade-in duration-300">
                    {[
                      { l: 'Motif / HDLM', v: s.hdlm, i: BookOpen },
                      { l: 'Tests Cliniques', v: s.tests, i: Activity },
                      { l: 'Traitement effectué', v: s.treatment, i: HeartPulse },
                      { l: 'Conseils', v: s.advice, i: ClipboardList }
                    ].map((item, i) => (
                      <div key={i} className="space-y-2">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed whitespace-pre-wrap">{item.v || "N/A"}</div>
                      </div>
                    ))}
                  </div>
                )}
