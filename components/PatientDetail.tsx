...
const InfoTag = ({ label, value, icon: Icon, href }: any) => {
  const displayValue = value && String(value).trim() !== "" && String(value) !== "undefined" ? value : "-";
  const Content = (
    <div className={`flex items-center gap-3 p-3.5 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800 transition-all ${href && displayValue !== "-" ? 'hover:border-primary group cursor-pointer' : ''}`}>
      {Icon && <Icon size={16} className={`shrink-0 ${href && displayValue !== "-" ? 'text-slate-400 group-hover:text-primary' : 'text-slate-400'}`} />}
      <div className="min-w-0 flex-1">
        <p className="text-[9px] font-medium text-slate-400 uppercase tracking-tighter mb-0.5">{label}</p>
        <p className={`text-xs font-semibold truncate ${href && displayValue !== "-" ? 'text-slate-700 dark:text-slate-200 group-hover:text-primary' : 'text-slate-700 dark:text-slate-200'}`}>
          {displayValue}
        </p>
      </div>
      {href && displayValue !== "-" && <ExternalLink size={10} className="text-slate-300 group-hover:text-primary opacity-0 group-hover:opacity-100 transition-all" />}
    </div>
  );

  return href && displayValue !== "-" ? (
...
const MedicalCard = ({ title, content, icon: Icon, iconColor = "text-primary/60" }: any) => (
  <div className="bg-white dark:bg-slate-900/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col gap-3 shadow-sm hover:border-slate-200 dark:hover:border-slate-700 transition-colors">
    ...
    <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed italic font-medium">{content && String(content).trim() !== "" ? content : "-"}</p>
  </div>
);
...
const PatientDetail: React.FC<PatientDetailProps> = ({ patientId, onEdit, onDelete }) => {
  ...
  const exportPDF = useCallback(() => {
    ...
    try {
      ...
      const sessionData = sessions.map(s => [new Date(s.date).toLocaleDateString(), s.hdlm || '-', s.treatment || '-', s.advice || '-']);
      autoTable(doc, {
        ...
        headStyles: { fillColor: primaryColor, textColor: 255 },
        columnStyles: { 0: { width: 25 }, 1: { width: 50 }, 2: { width: 50 }, 3: { width: 45 } }
      });
  ...
  if (loading) return <div className="py-24 flex justify-center"><Info className="animate-pulse text-primary" size={40} /></div>;
  if (!patient) return <div className="py-24 text-center text-slate-400">Patient introuvable</div>;
  ...
  const genderBorder = patient.gender === 'F' ? 'border-pink-500 shadow-[0_0_20px_rgba(236,72,153,0.3)]' : 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)]';
  const formattedGpName = patient.gpName && patient.gpName !== "Dr. " ? (patient.gpName.toLowerCase().startsWith("dr") ? patient.gpName : `Dr. ${patient.gpName}`) : "-";
  const smokerLabel = patient.isSmoker ? `Fumeur${patient.smokerSince ? ` (${patient.smokerSince})` : ''}` : patient.isFormerSmoker ? `Ancien fumeur${patient.smokerSince ? ` (${patient.smokerSince})` : ''}` : "Non fumeur";
  const familyLabel = patient.familyStatus ? `${patient.familyStatus}${patient.hasChildren && patient.hasChildren !== "Non" ? ` (${patient.hasChildren})` : ''}` : "-";
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-500">
      <div className="px-2 flex items-center justify-between">
        ...
        <button onClick={exportPDF} className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-primary hover:text-white rounded-xl text-[10px] font-bold uppercase tracking-widest text-slate-500 transition-all shadow-sm active:scale-95">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 sm:p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-xl flex flex-col lg:flex-row gap-10 items-center lg:items-start">
        ...
        <div className="flex-1 w-full text-center lg:text-left">
          <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-6 mb-8">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6 mt-8 p-5 bg-slate-50/50 dark:bg-slate-800/30 rounded-3xl border border-slate-100/50 dark:border-slate-800/50">
                <div className="flex flex-col gap-1"><span className="text-[10px] text-slate-400 font-medium uppercase tracking-widest leading-none">Dernière Séance</span><span className="text-sm font-bold text-primary">{stats.lastSessionDate ? new Date(stats.lastSessionDate).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }) : 'Première visite'}</span></div>
                <div className="w-px h-10 bg-slate-200 dark:bg-slate-700 hidden sm:block"></div>
                <div className="flex flex-col gap-1"><span className="text-[10px] text-slate-400 font-medium uppercase tracking-widest leading-none">Total</span><span className="text-sm font-bold text-slate-700 dark:text-slate-300">{stats.total} séance{stats.total > 1 ? 's' : ''}</span></div>
              </div>
            </div>
          ...
          <div className="mt-6 p-5 bg-slate-50/50 dark:bg-slate-800/30 rounded-3xl border border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center gap-4">
            <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center text-primary shrink-0"><Stethoscope size={20} /></div>
            <div className="flex-1 text-center sm:text-left min-w-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-2"><span className="text-sm font-bold text-slate-700 dark:text-slate-200 truncate">{formattedGpName}</span><span className="text-[10px] font-medium text-slate-400 uppercase tracking-tight">{patient.gpCity && `à ${patient.gpCity}`}</span></div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            <div className="p-6 bg-primary-soft border border-primary-border rounded-[2rem] shadow-sm flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-2 text-[10px] font-semibold text-primary uppercase tracking-widest px-1"><Activity size={16} /> Mode de vie</div>
            ...
            <div className="p-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/40 rounded-[2rem] shadow-sm flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-2 text-[10px] font-semibold text-amber-600 uppercase tracking-widest px-1"><HeartPulse size={16} /> Traitement actuel</div>
      ...
      <section className="pt-12 border-t border-slate-100 dark:border-slate-800 space-y-8">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2">
          <SectionHeader icon={BookOpen} title="Historique des consultations" />
          <button onClick={() => setIsAddingSession(true)} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-primary text-white px-8 py-3.5 rounded-2xl text-[11px] font-bold uppercase tracking-widest shadow-xl hover:brightness-105 transition-all">
              ...
              <div key={s.id} className={`bg-white dark:bg-slate-900 border rounded-[2rem] transition-all duration-300 ${open ? 'border-primary shadow-2xl scale-[1.01]' : 'border-slate-100 dark:border-slate-800 shadow-sm hover:border-slate-200'}`}>
                ...
                {open && (
                  <div className="px-8 pb-10 pt-6 grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-50 dark:border-slate-800 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="space-y-3"><label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest flex items-center gap-2">Motif / HDLM</label><div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl text-xs leading-relaxed border border-slate-100 dark:border-slate-800 italic">{s.hdlm || "-"}</div></div>
                    <div className="space-y-3"><label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest flex items-center gap-2">Traitement</label><div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl text-xs leading-relaxed border border-slate-100 dark:border-slate-800 italic">{s.treatment || "-"}</div></div>
                  </div>
                )}
              </div>
            );
          }) : (
            <div className="py-16 flex flex-col items-center text-slate-400 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-[3rem]"><History size={32} className="opacity-10 mb-4" /><p className="text-[10px] font-semibold uppercase tracking-[0.2em]">Aucune séance enregistrée</p></div>
          )}
        </div>
      </section>
