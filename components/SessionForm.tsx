      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        <div className="flex flex-col sm:flex-row gap-6">
          <div className="w-full sm:w-1/3 space-y-1.5">
            <label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest flex items-center gap-2 px-1"><Calendar size={12} /> Date Séance</label>
          ...
          <div className="flex-1">
            <TextAreaField label="Motif / HDLM" icon={BookOpen} required value={formData.hdlm} onChange={(v:any) => setFormData({...formData, hdlm: v})} />
          </div>
        </div>
        ...
        <button type="submit" className="w-full py-3.5 bg-primary text-white font-bold text-[11px] uppercase tracking-widest rounded-xl hover:brightness-105 active:scale-[0.99] transition-all shadow-lg flex items-center justify-center gap-2">
          <Save size={18} /> {sessionToEdit ? 'Mettre à jour' : 'Enregistrer'}
        </button>
      </form>
    </div>
  );
